...
### Added - 2026-07-31（Chapter 15：小蜜蜂射击游戏）

- 第五款学习游戏「小蜜蜂」：首款 Canvas 2D 游戏（方块飘落 + 虚拟键盘拼写 + 飞机射击 + 粒子爆炸动效）
- 难度三级（初级/中级/高级：方块数 2/4/6 + 下落速度 8/6/4 秒/屏，`config/gameRules.ts` 可调）
- QWERTY 虚拟键盘组件（WXML 按钮区，发射 keypress/backspace 事件）
- 全量复用：gameType='shooter'（零 DB 变更）、统一结果页、复习集成闭包
- shooterEngine 纯逻辑引擎（方块生成/移动/碰撞/输入匹配/粒子，全部可单测）
- 待补：音效与 BGM（Q4 V1 静音上线，素材后续添加）

### Added - 2026-07-31（教材选择模型定稿 + 导入器 upsert 重构）

- 教材选择三步流（Owner 定稿）：首次点学科必须选「版本 → 年级 → 学期」，之后默认；一个学科一种组合；选完回学习页并标注「版本｜年级｜学期」
- 年级展示层拆分（gradeService + 年级选择页）：数据层册次不动，浏览路径 学科→路径→版本→年级→学期
- 语法扁平化：点语法直达专题章节（按学段匹配语法专题包），跳过专题包/全册两层
- 乱码修复：教材名路由编码未解码入库（重新选择一次即刷新正常中文）
- 我的页新增「教材选择」显性入口；导入器重构为 upsert（id 稳定，用户数据跨导入存活）；陈旧偏好按名称自愈
- 学习 tab 升级「我的课程」：单词+语法双分区，顶部标注 版本｜年级｜学期

### Added - 2026-07-30（Chapter 14：教材偏好 + 学习 tab 改造）

- `users.preferences`（教材版本+册次+名称冗余；`updateUserPreferences` 云函数收口写权限）
- 偏好自动保存：点开任意册次即视为当前偏好（无需确认按钮，再点自然覆盖）
- 学习 tab 改造「我的教材」：已设偏好直达当前册次章节列表（顶部教材横条可切回选择流）；未设偏好显示引导卡
- 首页引导卡：未设偏好时出现「选你的教材和册次」
- `chapterService.buildChapterItems` 抽取共用（章节页/学习 tab DRY）；`shared/chapter-items.wxml` 章节列表共享模板
- 单测 +3（状态映射/入口显隐/游戏门槛）

### Added - 2026-07-30（Chapter 13：四维单元测试 一期 + 阅读集合特批）

- 单元测试升级综合测评卷（一期）：听力 2（TTS 自动播放+重播，题型标签 Section 化）+ 单词 8，共 10 题；`testService.buildPaper` + `config/testPaper.ts` 阶段化配比
- ADR-007：`reading_passages` 第 17 集合特批（阅读理解测评素材，二期）
- 二期骨架：语法题 48 道（24 点 × 2，`grammar-quiz-draft.csv` 待校对）+ 阅读样张 2 篇（`reading-sample-draft.csv` 定格式）
- 单测 +4（组卷构成/不重复/语法排除/小池兜底）

### Added - 2026-07-20（Chapter 12：英语语法内容线 + 数据质量修复）

- 边界修订（2026-07-30，Owner）：语法不需要朗读与游戏——学习详情对语法点隐藏发音按钮；selectPool（全部游戏）与 testService（单元测试）统一过滤为 type='word'；复习/收藏对语法点保留

- 语法学习路径点亮：Knowledge 双形态（type: 'word'|'grammar' + explanation 字段，只增不改）；首批 24 个语法点（小学 12 + 初中 12，时态/词法/句型各 8 专题，AI 骨架待校对）
- 学习详情新增语法讲解区块（explanation 存在时展示）
- 数据质量三连修：① 转换器改按各文件表头映射（修复语法列被首文件表头丢弃）；② 转换器新增列数不一致警告（揪出 40 行历史错列：example 列未引号逗号，已批量修复）；③ 八上 Unit 10 标题逗号导致整单元 12 行错列（标题去逗号，数据还原）
- 附注：翻译双雄（Chapter 11）经 Owner 取消（与极速选择重叠），草案归档于 docs/archive/
