...
## 基线（2026-07-19 确立；同日经 ADR-004 调和）

```
supereasystudy/
├── docs/                          # 文档区
│   ├── OPENCODE_RULES.md          #   开发宪法（最高优先级，Owner 维护）
│   ├── SuperEasy-Learning-Specification.md  # 产品 SSOT（Owner 维护）
│   ├── DEVELOPMENT_BASELINE_SPEC.md         # 开发基线规范（Mandatory，Owner 维护）
│   ├── architecture/              #   工程架构文档
│   ├── decisions/                 #   ADR 与待决事项
│   ├── guides/                    #   开发指南
│   └── archive/                   #   历史文档归档（仅供参考）
...
│   ├── subjects/                  #   学科插件目录
│   ├── pages/                     #   页面层（薄，≤300 行/文件；当前 17 个空占位页）
...
├── cloud/
│   ├── functions/                 # 云函数（cloudfunctionRoot；每函数独立 package.json）
│   └── database/                  # 数据库集合基线（Baseline Spec §4）
├── scripts/                       # 工程脚本（Node，不进小程序运行时）
├── tests/                         # Vitest 单元测试
├── typings/                       # 项目级全局类型
├── .github/workflows/             # CI
...
| `utils/`           | 纯函数                    | 依赖任何上层目录                    |
| `cloud/functions/` | 云函数（DB 写、权限校验） | 依赖客户端上下文                    |
| `cloud/database/`  | 集合基线清单与说明        | 修改集合名称（Spec §4 红线）        |
| `scripts/`         | 工程自动化脚本            | 产品业务逻辑                        |
