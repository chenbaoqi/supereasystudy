...
> 产品依据：`../SuperEasy-Learning-Specification.md`（SSOT）
> 行为准则：`../OPENCODE_RULES.md`（优先级最高）
> 基线规范：`../DEVELOPMENT_BASELINE_SPEC.md`（Mandatory）

## 1. 产品定位对工程的要求

| Specification 要求         | 工程对策                                            |
| -------------------------- | --------------------------------------------------- |
...
| 激励广告变现               | 配置化广告位（`config/features.ts`，接口预留）      |

## 2. 仓库全景

```
supereasystudy/
├── docs/                # SSOT 文档区（RULES + Specification + BaselineSpec + 架构 + ADR + 指南 + 归档）
├── miniprogram/         # 微信小程序端（原生 + TypeScript）
│   ├── core/            #   领域抽象（接口/类型，零实现，零运行时依赖）
│   ├── subjects/        #   学科插件（V1: english）
│   ├── pages/           #   页面层（薄；当前 17 个空占位页，Baseline Spec §3）
│   ├── components/      #   通用展示组件
│   ├── services/        #   业务逻辑层（纯 TS，可单测）
│   ├── repositories/    #   数据访问层（唯一 wx.cloud 封装点）
│   ├── config/          #   配置（云环境/功能开关/学科，配置优先）
│   └── utils/           #   纯函数工具
├── cloud/
│   ├── functions/       # 微信云函数（每个函数独立 package.json）
│   └── database/        # 数据库集合基线（14 集合，Spec §4）
├── scripts/             # 工程脚本
├── tests/               # 单元测试（Vitest）
├── typings/             # 项目级全局类型
...
| `decisions/`                          | ADR-001~004 + 待决事项             |
| `guides/`                             | 开发环境、编码规范、Git 提交规范   |
| `archive/`                            | 历史文档（仅供参考，不作开发依据） |
